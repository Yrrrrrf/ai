...
class PSO:
    ...
    def _initialize_swarm(self):
        ...
        self.positions = np.random.uniform(low, high, (self.num_particles, self.dimensions))
        
        # Initialize velocities randomly within a fraction of the bounds range
        velocity_range = (high - low) * 0.1
        self.velocities = np.random.uniform(-velocity_range, velocity_range, (self.num_particles, self.dimensions))
        
        # Evaluate initial positions
        self.fitness_values = np.array([self.fitness_function(pos) for pos in self.positions])
    ...
    def visualize_particles_2d(self, interval=20, save_path=None):
        ...
        # Create animation
        animation = FuncAnimation(fig, update, frames=len(self.positions_history),
...
# Demo: Solve 2D Rastrigin function optimization
def main():
    # Problem parameters
    num_particles = 30
    dimensions = 2
    ...
    # Visualize particle movements (only for 2D problems)
    if dimensions == 2:

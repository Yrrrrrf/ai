...
> Schema for the Karpathy 3-layer knowledge territory. Inherits: root
> `AGENTS.md`.
...
- Organized by topic: `cv/`, `evo/`, `ga/`, `neural-net/`, `notebooks/` (orphan
  notebooks), `sys/` (system diagnostics).

### `wiki/` — Synthesized knowledge

- **One topic per file.** Filename is a kebab-case slug of the topic.
- **Cite sources.** Every wiki page should reference its `raw/` sources at the
  bottom of the page.
- **Update index.** `wiki/index.md` maps every slug to its human-readable topic
  name. Update it on every page add or rename.
...
**deferred**. For v1.0, maintenance is manual. This decision will be revisited
after 1–2 months of use to assess whether automation is warranted based on

...
def create_random_tsp_instance(num_cities: int, max_coordinate: float = 100.0) -> TSPData:
    """
...
def orderx_crossover(parent1: torch.Tensor, parent2: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
    """
...
def plot_tsp_route(cities: torch.Tensor, route: List[int], city_names: List[str] = None, title: str = "Ruta del viajante"):
    """
    ...
    route_x = [x[route[i]] for i in range(len(route))] + [x[route[0]]]  # Volver al inicio
    ...
    if city_names:
        for i, city in enumerate(cities):
            plt.annotate(city_names[i], (x[i], y[i]), xytext=(5, 5), 
...
def display_tsp_solution(best_chromosome: torch.Tensor, problem_data: TSPData):
    ...
    for i, city_idx in enumerate(best_route):
        city_name = problem_data.city_names[city_idx] if problem_data.city_names else f"Ciudad {city_idx + 1}"
        next_city_idx = best_route[(i + 1) % num_cities]
        next_city_name = problem_data.city_names[next_city_idx] if problem_data.city_names else f"Ciudad {next_city_idx + 1}"
        distance = problem_data.distances[city_idx, next_city_idx].item()
        print(f"{i+1}. {city_name} → {next_city_name} ({distance:.2f})")
    
    # Visualizar la ruta
    plot_tsp_route(problem_data.cities, best_route, problem_data.city_names, 
...
def solve_tsp():
    ...
    plot_tsp_route(problem_data.cities, list(range(num_cities)), 
                  problem_data.city_names, title="Disposición de las ciudades")

...
name: lucide
description: >-
  Use this skill when adding icons to a Svelte 5 application with @lucide/svelte.
  Covers 1694 stroke-style SVG icons with runtime-tunable size, color, and
  stroke-width. Reach for this skill whenever the task involves icon selection,
  tree-shaken icon imports, global icon defaults via context, accessible icon
  labeling, dynamic icon rendering by name, or absoluteStrokeWidth scaling.
  Also use when debugging icon class composition, aria-hidden behavior, or
  iconNode custom shapes.
---
# [[lucide]] [[agent#skills|skills]]

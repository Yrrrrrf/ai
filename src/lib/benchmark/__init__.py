...
def run_convergence_analysis(algorithm_class: Type[OptimizableAlgorithm]):
    ...
    for prob in benchmark_problem_library:
        try:
            ...
            ui.print_message(f"  Finished. Best fitness: {algo_instance.best_fitness_history[-1]:.6f}", style="green")

        except NotImplementedError:
            # This is the designed way to skip incompatible algorithm/problem pairs
            continue
        except Exception as e:
            ui.print_message(f"Error on [bold]{prob.name}[/bold]: {e}", style="bold red")

    # --- Phase 2: Create the final summary plot ---
    if not results:
        ui.print_message("No compatible problems found or all runs failed.", style="bold red")
        return
    ...
    fig.suptitle(f"{algorithm_class.__name__} Convergence Analysis", fontsize=16, weight="bold")
    axes = axes.flatten()
...
def compare_hyperparameters(algorithm_class: Type[OptimizableAlgorithm]):
    ...
    if not compatible_problems:
        ui.print_message(f"No problems in the library are configured for {algo_name}.", style="red")
        return

    ui.console.print(f"\n[bold underline]Select a Problem for {algo_name} Hyperparameter Comparison[/]", justify="center")
...
def view_pso_live_animation():
    ...
    runnable_problems = [
        p for p in benchmark_problem_library if '2d' in p.tags and isinstance(p, ContinuousProblem)
    ]
...
BENCHMARK_SUITES: Dict[str, List[BenchmarkSuite]] = {
    "PSO": [
        ("Run batch convergence analysis on all problems", lambda: run_convergence_analysis(PSO)),
        ("Compare hyperparameters on a chosen problem", lambda: compare_hyperparameters(PSO)),
        ("View a live 2D animation for a chosen function", view_pso_live_animation),
    ],
    "Genetic Algorithm": [
        ("Run batch convergence analysis on all problems", lambda: run_convergence_analysis(GeneticAlgorithm)),
        ("Compare hyperparameters on a chosen problem", lambda: compare_hyperparameters(GeneticAlgorithm)),
    ],
}
...
def run() -> None:
    ...
    while True:
        ...
        ui.display_table("Select an Algorithm to Test", ["Choice", "Algorithm"], menu_rows)

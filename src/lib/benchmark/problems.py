...
class ContinuousProblem(BenchmarkProblem):
    ...
    def __init__(self, name: str, func: callable, bounds: Tuple[float, float], dimensions: int, tags: list[str] = None):
...
class KnapsackProblem(BenchmarkProblem):
    ...
    def __init__(self):
        super().__init__(name="Knapsack", minimize=False, tags=['combinatorial', 'binary'])
        self.problem_data = {
    ...
    def get_config_for_algorithm(self, algo_name: str) -> Dict[str, Any]:
        ...
        raise NotImplementedError(f"The Knapsack problem is not configured for the '{algo_name}' algorithm.")
...
benchmark_problem_library = [
    ...
    ContinuousProblem(name="Sphere (2D)", func=sphere_func, bounds=(-5.12, 5.12), dimensions=2, tags=['2d']),
    ContinuousProblem(name="Rosenbrock (2D)", func=rosenbrock_func, bounds=(-5.0, 10.0), dimensions=2, tags=['2d']),
    ContinuousProblem(name="Ackley (2D)", func=ackley_func, bounds=(-32.768, 32.768), dimensions=2, tags=['2d']),
    ContinuousProblem(name="Griewank (2D)", func=griewank_func, bounds=(-600.0, 600.0), dimensions=2, tags=['2d']),
    ContinuousProblem(name="Schwefel (2D)", func=schwefel_func, bounds=(-500.0, 500.0), dimensions=2, tags=['2d']),
    ContinuousProblem(name="Rastrigin (2D)", func=rastrigin_func, bounds=(-5.12, 5.12), dimensions=2, tags=['2d']),
    ContinuousProblem(name="Himmelblau (2D)", func=himmelblau_func, bounds=(-5.0, 5.0), dimensions=2, tags=['2d']),
    ContinuousProblem(name="Easom (2D)", func=easom_func, bounds=(-100.0, 100.0), dimensions=2, tags=['2d']),
    
    # We can also add higher-dimensional problems easily
    ContinuousProblem(name="Sphere (10D)", func=sphere_func, bounds=(-5.12, 5.12), dimensions=10),

...
class GeneticAlgorithm:
    def __init__(self, 
        ...
        if binary:
            self.population = torch.randint(0, 2, (pop_size, chromosome_length), dtype=torch.float32)
        else:
            self.population = torch.rand((pop_size, chromosome_length), dtype=torch.float32)
    ...
    def run(self, generations: int, verbose: bool = True) -> Tuple[torch.Tensor, float]:
        ...
        for gen in range(generations):
            ...
            if verbose and ((gen + 1) % 10 == 0 or gen == 0 or gen == generations - 1):
                print(f"Generación {gen + 1}: Mejor fitness = {best_fitness:.2f}, Fitness promedio = {self.avg_fitness_history[-1]:.2f}")
